...
def build_conv_layer() -> tf.keras.layers.Conv2D:
    ...
    my_layer = tf.keras.layers.Conv2D(filters=filters, kernel_size=kernel_size, strides=strides)
...
def build_padded_conv_layer(kernel_size) -> tf.keras.layers.Conv2D:
    ...
    my_layer = tf.keras.layers.Conv2D(filters=filters, kernel_size=kernel_size,padding='same')
